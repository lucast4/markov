...
# temp_list = [1, 1, 1, 1]
K_list = [3, 3]  # number of states
N_list = [10, 10]  # size of states
Nhid_list = [50, 50] # size of hidden layer
...
savesuffix_list = ["figure"] # can be empty
freezeWeights_list = [True]
nrepeats = 1
doBPTT = True
doanneal = True # if true, goes from temp to 1e-4. if False then uses just temp throughout.
makedifferent = True # if false, then random matrices. If true, then makes them different [only if 2 contexgts]
...
trainUsingContext_list = [True, False] # false means only observe states. True means observes context + state.
...
for K, N, Kc, Nhid, ntrain, temp in zip(K_list, N_list, Kc_list, Nhid_list, ntrain_list, temp_list):
        ...
        for _ in range(nrepeats):
                for savesuffix, freezeWeights in zip(savesuffix_list, freezeWeights_list):
                        for trainUsingContext in trainUsingContext_list:
